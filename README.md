...
This writes an autotakeout-specific Backrest config under `~/.local/state/autotakeout/backrest/` and starts the UI at `http://127.0.0.1:9898`. The backup plan points at the merged Google Photos directory; schedules are disabled so Backrest will not start its own backup while this script is already uploading.
